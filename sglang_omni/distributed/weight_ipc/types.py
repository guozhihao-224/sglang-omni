...
class IpcTensorMeta:
    """Metadata for one shared CUDA tensor.

    ``handle`` / ``storage_*`` / ref-counter / event fields come from PyTorch's
    ``UntypedStorage._share_cuda_()`` (CUDA IPC under the hood, not
    ``ForkingPickler``).

    ``allocation_offset_bytes`` is ``data_ptr - cuMemGetAddressRange.base`` and
    is recorded for diagnostics and tests (caching-allocator suballocation).
    """
