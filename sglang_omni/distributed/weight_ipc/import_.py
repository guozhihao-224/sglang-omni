...
def import_and_alias(
    ...
    *,
    ...
    Returns a name→tensor map that the caller must retain for the process
    lifetime so imported storages are not garbage-collected.
    """
    ...
    for meta in bundle.tensors:
        ...
        if isinstance(local, torch.nn.Parameter):
            ...
            # Buffer path (whitelist only): replace storage via copy of meta alias.
            local.set_(remote)
        imported[meta.name] = remote

    # Strong refs so GC cannot reclaim imported storages while params alias them.
