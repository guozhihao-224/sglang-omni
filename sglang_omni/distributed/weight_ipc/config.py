...
def resolve_weight_ipc_config(
    ...
    """Return the active weight-IPC config.

    Precedence: explicit argument, then environment variables, else ``role=off``.
    """
...
def apply_weight_ipc_cli_env(
    *,
    ...
    # Validate early so serve fails before spawning stages.
