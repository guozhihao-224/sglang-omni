...
Weight IPC intentionally avoids ``ForkingPickler`` / ``relay_io.ipc_pickle``.
Tensor reconstruction uses PyTorch's ``UntypedStorage._share_cuda_`` /
``_new_shared_cuda`` API, which wraps ``cudaIpc*`` plus ref-counting required
for safe cross-process storage lifetime.

Raw ``cudaIpcOpenMemHandle`` + ``UntypedStorage._new_with_weak_ptr`` was
prototyped and segfaults on current PyTorch; do not revive that path.
"""
