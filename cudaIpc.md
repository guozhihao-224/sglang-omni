...
| Phase | 内容 | Exit |
|---|---|---|
| 0 | `cuda_handles` + CUDA e2e unit (`test_weight_ipc_cuda.py`) | ✅ 已完成 |
...
**Phase 3 多轮 @ Equal KV=100000**（`TRIALS=3`，`SAMPLES=100` / `TTFC_SAMPLES=150`，产物 `results/weight_ipc_phase3_multi_100000_20260718-123321/`；**逐 trial / 逐次探测全表**见 [`results/weight_ipc_h100_report_20260718.md`](results/weight_ipc_h100_report_20260718.md)）：

| Config | QPS mean ± std | TTFC p99 (s) |
|---|---:|---:|
| DP3 unshared peak | 32.74 ± 0.25 | — |
| DP3 shared peak | 32.78 ± 0.23 | — |
| DP3 shared @ ~34 offered | 31.31 ± 0.71 | 0.77 ± 0.15 |
| DP4 shared peak | **39.46 ± 0.76** | 5.49 ± 0.08（饱和） |
| DP4 shared @ ~34 offered | 30.28 ± 2.25 | **0.59 ± 0.06** |

| Trial | Δ% (S−U) | G2 | DP4 peak | DP4@34 QPS / p99 |
|---:|---:|---|---:|---|
| 1 | −1.33 | ✓ | 39.32 | 32.87 / 0.64 |
| 2 | +0.86 | ✓ | 38.78 | 28.91 / 0.52 |
| 3 | +0.85 | ✓ | 40.28 | 29.06 / 0.62 |

- shared−unshared：**+0.12% ± 1.26%**；**G2 3/3 pass**
- DP4 峰值相对 DP3 shared 约 **+20%**；matched-load TTFC 多 trial 均值 DP4 优于 DP3；`@34` QPS 波动见完整报告 trial 明细

**Equal-KV 上探（同日，干净 H100 GPU3）** — `examples/weight_ipc/probe_equal_kv.sh`，产物 `results/weight_ipc_kv_probe_gpu3_20260718-122044/`（汇总 `results/weight_ipc_kv_probe_20260718/SUMMARY.md`；全部 23 次探测明细见完整报告）：

| Equal KV T | 结果 | 卡显存 (MiB) | 说明 |
|---:|---|---:|---|
| 100000 | **PASS** | 70326 / 81559 | recipe；约 11GB 余量 |
| 110000 | **PASS** | 76086 / 81559 | 约 5.3GB 余量 |
| 112000 | **PASS** | 77238 / 81559 | 干净复测最高 PASS |
| 113000 | **FAIL** | — | replica 2 启动 OOM / CUDA error |
| 115000 | **FAIL** | — | Equal-KV 解析约 112.8k |

结论：共享权重把 DP4 Equal KV 从约 70k 抬到 **`T_max=112000`**；生产仍建议 **100000**（余量），需要更大 KV 时可试 110k–112k；≥113k 需主机实测，≥115k 本配置不可行。
...
GPU_ID=3 N=4 CORE_BLOCKS="0-7 8-15 16-23 24-31" BASE_PORT=8901 \
  TOKENS_LIST="100000 110000 112000 113000 115000" \
