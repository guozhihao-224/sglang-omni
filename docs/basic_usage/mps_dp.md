...
Helpers for correctness, Phase-3 speed checks, and Equal-KV probing:

```bash
bash examples/weight_ipc/run_h100_go_nogo.sh dp2-parity
bash examples/weight_ipc/run_phase3_multi.sh   # TRIALS=3 @ MAX_TOTAL_TOKENS=100000; needs CORE_BLOCKS_DP3/DP4
# Optional: raise MAX_TOTAL_TOKENS until Equal-KV fails
TOKENS_LIST="100000 110000 112000 113000 115000" \
  bash examples/weight_ipc/probe_equal_kv.sh
```

On H100 Higgs at Equal KV=100000, a 3-trial Phase-3 matrix (`results/weight_ipc_phase3_multi_100000_20260718-123321/`) measured DP3 shared ≈ DP3 unshared (G2 3/3; mean +0.1%), DP4 shared peak ≈ **39.5 QPS** (~+20% vs DP3 shared), and matched ~34 offered TTFC p99 mean **0.59s** (DP4) vs **0.77s** (DP3). Card memory for DP4 shared @ 100k is about 70–71 GB. Default remains `WEIGHT_IPC=0`.

On the same H100 Higgs DP4 + `WEIGHT_IPC=1` setup, a clean Equal-KV probe found **`T_max = 112000`** (100k/110k/112k PASS; 113k+ FAIL). Keep the recipe at `100000` for headroom; see [`results/weight_ipc_kv_probe_20260718/SUMMARY.md`](../../results/weight_ipc_kv_probe_20260718/SUMMARY.md).
